...
Downloaded the data to my drive from here:
Pulled from [here](https://divvy-tripdata.s3.amazonaws.com/index.html)
...
```{r}
all_trips_clean %>%
group_by(usertype = "member", start_station_name) %>% 
summarise(number_of_rides = n()) %>% 
...
```{r}
all_trips_clean %>%
group_by(usertype = "casual", start_station_name) %>% 
summarise(number_of_rides = n()) %>% 
arrange(desc(number_of_rides)) %>%
group_by(usertype) %>%
slice(1:5)
...
```{r}
all_trips_clean %>%
group_by(usertype = "member", end_station_name) %>% 
summarise(number_of_rides = n()) %>% 
arrange(desc(number_of_rides)) %>%
group_by(usertype) %>%
slice(1:5)
...
```{r}
all_trips_clean %>%
group_by(usertype = "casual", end_station_name) %>% 
summarise(number_of_rides = n()) %>% 
arrange(desc(number_of_rides)) %>%
group_by(usertype) %>%
slice(1:5)
